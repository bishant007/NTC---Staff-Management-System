...
function SectionHeadPending() {
  ...
  const submitApprove = async () => {
    ...
    try {
      ...
      setMessage('✅ Approved and forwarded to Department Head');
      closeAction();
      load();
    } catch (err) {
  ...
  const submitReject = async () => {
    ...
    try {
      await rejectBySectionHead(activeAction.id, rejectionReason);
                  ...
                  {activeAction.type === 'approve' ? (
                    <>
                      ...
                      <textarea rows={2} value={notes} onChange={e => setNotes(e.target.value)}
                        placeholder="Any remarks for the department head" style={{ ...inp, resize: 'vertical' }} />
