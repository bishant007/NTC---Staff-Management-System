...
function SectionHeadHistory() {
  ...
  const filtered = requests.filter(r => {
    ...
    if (filter === 'ACTED_BY_ME') return r.sectionHeadApprovedAt !== null;
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: '#eef4ff' }}>
      ...
      <div style={{ flex: 1, marginLeft: 250, padding: '30px' }}>
        ...
        <p style={{ color: '#5b7bab', marginBottom: 24 }}>
          All requests from your staff that you have acted on (or that are still pending).
        </p>

        <div style={{ display: 'flex', gap: 10, marginBottom: 20, flexWrap: 'wrap' }}>
          {['ALL', 'ACTED_BY_ME', 'PENDING_SECTION_HEAD', 'PENDING_DEPARTMENT_HEAD', 'APPROVED', 'REJECTED'].map(f => (
        ...
        {filtered.length === 0 ? (
          ...
          filtered
            ...
            .map(r => <RequestDetail key={r.id} request={r} viewerRole="section_head" />)
        )}
      </div>
    </div>
  );
